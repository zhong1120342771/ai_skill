---
name: moscow-defense-monthly-biz
version: 1.0.0
description: "莫斯科保卫战月报编排器：通过 Agent 工具串行调度 5 个独立 sub-agent（代码生成→数据分析→质量检查→洞察结论生成，内嵌数据可视化），每步上下文隔离、产物落盘后才进下一步，自动产出飞书文档 + P2P 推送到钟梦婷。应用场景为每月月初分析上一个完整自然月的数据。当用户说「莫斯科保卫战月报」「保卫战月报」「莫斯科月报」「月报数据整理」「跑一下上月保卫战数据」「更新 YYYY-MM 的月报」时必须调用本 skill。"
metadata:
  type: orchestrator
  pipeline:
    - 代码生成
...
**应用场景**：每月月初，分析上一个完整自然月（如 7 月初跑 6 月）的莫斯科保卫战数据。数据模块、重点指标、拆解维度、可视化基本对齐周报（`moscow-defense-weekly-biz`），仅把「周」口径改为「月」，并新增「同比」。

## 唤醒关键词

- 莫斯科保卫战月报 / 保卫战月报 / 莫斯科月报
- 月报制作 / 月报数据整理 / 莫斯科月度数据整理
- 更新 YYYY-MM 的月报数据 / 跑一下上月保卫战数据

## 目录布局

```
moscow-defense-monthly-biz/
├── SKILL.md                    # 本编排器
...
│   ├── 01_panel_monthly.sql    # 月度全维度面板（核心，覆盖 KPI/漏斗/5维度/大盘图）
...
产物落 `~/Downloads/msk_monthly_raw/${month}/`，文件名见各 agent。
...
Agent tool:
  ...
  prompt: "month = ${month}
    month_end = ${month_end}
    你的 agent 定义文件路径: /Users/zhongmengting/.claude/skills/moscow-defense-monthly-biz/agents/代码生成.md
    请完整 Read 该文件并严格执行其中的所有指令。"
```

**产物校验**（缺一个即停）：
```bash
DIR=~/Downloads/msk_monthly_raw/${month}
...
```
Agent tool:
  description: "Step 2 数据分析 - 漏斗框架 + 5 维度拆解 + 月环比/同比"
  prompt: "month = ${month}
    你的 agent 定义文件路径: /Users/zhongmengting/.claude/skills/moscow-defense-monthly-biz/agents/数据分析.md
    请完整 Read 该文件并严格执行其中的所有指令。"
```
...
Agent tool:
  ...
  prompt: "month = ${month}
    你的 agent 定义文件路径: /Users/zhongmengting/.claude/skills/moscow-defense-monthly-biz/agents/质量检查.md
    请完整 Read 该文件并严格执行其中的所有指令。"
```
...
Agent tool:
  ...
  prompt: "month = ${month}
    你的 agent 定义文件路径: /Users/zhongmengting/.claude/skills/moscow-defense-monthly-biz/agents/洞察结论生成.md
    请完整 Read 该文件并严格执行其中的所有指令。"
```
...
- **数据源硬约束**：莫斯科归因走 `hdp_zhuanzhuan_tmp_global.tmp_dws_msk_zhibiao_zmt_v2_di`，**禁用** `tmp_dws_msk_zmt_app_v2_di`（同维度名但数差 5–10%）。月报只用这一张归因表出 01_panel + 02_daily。
...
- **落位与周报一致**：wiki space 7639643477596441545，父节点 PGlqwRjBIivjKnkInplcEEQ6ndg，标题「【${month_cn}】莫斯科保卫战月报」。
...
| 维度 | 周报（moscow-defense-weekly-biz） | 月报（本 skill） |
|------|------|------|
| 周期 | 周一–周日完整周 | 完整自然月 |
| 对比口径 | 周环比 | 月环比 + 同比 |
| 取数 SQL | 11 段（含易错 union all 窗口 + 01_kpi NPE） | 2 段（月度面板 01_panel + 02_daily，规避 NPE） |
| KPI 表列 | 指标/月度目标/本月均值/本周均值/周环比 | 指标/月度目标/本月均值/月环比/月同比 |

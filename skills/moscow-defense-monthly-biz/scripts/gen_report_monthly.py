...
2. 生成 KPI 表（指标/月度目标/本月均值/月环比/月同比）、整体漏斗横表、5 维度 7 列表（单元格双拼环比｜同比）、维度构成表的 XML
...
# ====== KPI 表（5 列：指标 / 月度目标 / 本月均值 / 月环比 / 月同比） ======

# 业务目标（与周报同一套；如口径变化在此调整）
KPI_TARGETS = {
    'dau-净支付pv转化率': ('净支付转化率', 1.26, True),   # 目标 1.26%
    'dau_日均':          ('DAU（日均）', 4500000, False), # 450 万
    '单量':              ('净支付单量（月均）', 56674, False),
}


def build_kpi_table(idx, month):
    """KPI 表 XML（5 列：指标/月度目标/本月均值/月环比/月同比）。数据源：整体行"""
    header = ['指标', '月度目标', '本月均值', '月环比', '月同比']
    hcells = ''.join(f'<th background-color="light-gray"><p>{h}</p></th>' for h in header)
    rows_xml = []
    for col, (label, target, is_rate) in KPI_TARGETS.items():
        cur_abs, mom, yoy = metric_with_changes(idx, '整体', '整体', month, col, is_rate)
        target_str = f'{target:.2f}%' if is_rate else f'{int(target):,}'
        ...
        cells = ''.join(f'<td><p>{c}</p></td>' for c in
                        [label, target_str, cur_disp, mom_str, yoy_str])
...
def _dim_items(idx, tag, month):
    """返回该维度本月出现的所有 wd。用户资产分层按 Z0-Z5 排序，其余按面板出现顺序。

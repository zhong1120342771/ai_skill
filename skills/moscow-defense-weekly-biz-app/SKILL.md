---
name: moscow-defense-weekly-biz-app
version: 1.0.0
description: "莫斯科保卫战周报编排器（仅 App 端）：数据源为 App 端专属表 tmp_dws_msk_zmt_app_v2_di，无「端」维度，用品类/场景/用户来源/用户资产分层四维拆解。通过 Agent 工具串行调度 4 个独立 sub-agent（代码生成→数据分析→质量检查→洞察结论生成），每步上下文隔离、产物落盘后才进下一步，自动产出飞书文档 + P2P 推送到钟梦婷。当用户说「莫斯科保卫战周报_仅app端」「仅app端莫斯科周报」「app端保卫战周报」「app端莫斯科周报」「仅看app端的莫斯科周报」时必须调用本 skill。多端口径（含小程序/找靓机）的周报走 moscow-defense-weekly-biz。"
metadata:
  type: orchestrator
  pipeline:
    - 代码生成
...
# 莫斯科保卫战周报 · 仅 App 端（编排器）

> 数据源：App 端专属表 `hdp_zhuanzhuan_tmp_global.tmp_dws_msk_zmt_app_v2_di`。无「端」维度，拆解走品类/场景/用户来源/用户资产分层四维。多端口径周报请走 `moscow-defense-weekly-biz`。
...
│   ├── 03_dim_funnel_scene_source.sql
...
产物落 `~/Downloads/msk_weekly_raw_app/${week_end}/`，文件名见各 agent。
...
Agent tool:
  ...
  prompt: "week_end = ${week_end}
    ...
    你的 agent 定义文件路径: /Users/zhongmengting/.claude/skills/moscow-defense-weekly-biz-app/agents/代码生成.md
    请完整 Read 该文件并严格执行其中的所有指令。"
```

**产物校验**（编排器逐项检查，缺一个即停）：
```bash
DIR=~/Downloads/msk_weekly_raw_app/${week_end}
for f in 01_kpi.csv 02_funnel.csv 03_dim_ss.csv 04_dim_ac.csv \
...
Agent tool:
  ...
  prompt: "week_end = ${week_end}
    week_start = ${week_start}
    你的 agent 定义文件路径: /Users/zhongmengting/.claude/skills/moscow-defense-weekly-biz-app/agents/数据分析.md
    请完整 Read 该文件并严格执行其中的所有指令。"
```
...
Agent tool:
  ...
  prompt: "week_end = ${week_end}
    week_start = ${week_start}
    你的 agent 定义文件路径: /Users/zhongmengting/.claude/skills/moscow-defense-weekly-biz-app/agents/质量检查.md
    请完整 Read 该文件并严格执行其中的所有指令。"
```
...
Agent tool:
  ...
  prompt: "week_end = ${week_end}
    week_start = ${week_start}
    你的 agent 定义文件路径: /Users/zhongmengting/.claude/skills/moscow-defense-weekly-biz-app/agents/洞察结论生成.md
    请完整 Read 该文件并严格执行其中的所有指令。"
```
...
|---|---|---|---|---|
| 1 | 代码生成 | week_end / week_start | `~/Downloads/msk_weekly_raw_app/${week_end}/*.csv`（11 个） | 全部存在且非空 |
...
- **数据源硬约束**：App 端保卫战走 `hdp_zhuanzhuan_tmp_global.tmp_dws_msk_zmt_app_v2_di`（App 端专属表，唯一主源）。本表无「拆分端」维度，用「品类/场景/用户来源/用户资产分层」四维拆解。多端归因表 `tmp_dws_msk_zhibiao_zmt_v2_di` 是另一支周报（moscow-defense-weekly-biz）的源，本 skill 不用。
...
| 报告里保留「端」拆解章节 | 用品类/场景/用户来源/用户资产分层四维（App 端无「端」） |
...
| 日期 | 周报区间 | 飞书文档 | 备注 |
|------|---------|---------|------|
| — | — | — | 仅 App 端周报，从多端周报 skill 派生（换 App 专属表 + 去端 + 四维拆解） |

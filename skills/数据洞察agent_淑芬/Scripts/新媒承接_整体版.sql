-- 【整体版 v5】业务重点名单(消费电子/二奢/兴趣) × 平台首次意向标签 流向分布
-- ⚠️换 dt 复用时：事件表(dw_log_server_action_1d)写死的 '2026-07-27' 与 cohort CTE 的
--   date_sub(current_date(),1) 必须指向同一天。原始跑通场景是 2026-07-28 运行(t-1=07-27)。
--   跑别的日期请把两处都对齐(要么全写死目标 dt，要么全用 date_sub)。
-- 口径：单日 dt='2026-07-27'

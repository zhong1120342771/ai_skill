--首页曝光事件
SELECT
    '首页（G1001）曝光事件' as tag
    ,dt
    ,token
    ,pagetype
    ,datapool['sectionId']  as sectionId -- 区域id
    ,datapool['subSectionId']  as subSectionId -- 子区域id
    ,datapool['subSectionName'] as subSectionName --子区域名称(2026-06-17 新增)
...
where
    ...
    and pagetype in ('zpmshow','Areaexposure','explosureGoods','explosureItems')  and actiontype in  ('G1001')
;

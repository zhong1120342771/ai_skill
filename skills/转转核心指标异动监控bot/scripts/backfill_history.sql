    -- ============================================================================
    -- backfill_history.sql — 底表历史回刷（30天滚动窗口）
    --   源自 DDL 真源 aaa全局脚本测试.sql 的 --03 回刷版；唯一改动=info CTE 钉最新快照(见下)。
    --   ${outFileSuffix} 为窗口右端日期；单批回刷 [date_sub(outFileSuffix,30), outFileSuffix]。
    --   动态分区 PARTITION(dt)，逐批推进覆盖 2025-01-01 至今。
    --   由 run_backfill.py 按批替换 ${outFileSuffix} 提交星河(Hive engine=5)。
    -- ============================================================================
    ...
    dau as (
        ...
        where t3.dt between date_sub('${outFileSuffix}',30) AND '${outFileSuffix}'
    ...
    (--raw流量漏斗
        SELECT --曝光
            ...
            WHERE dt between date_sub('${outFileSuffix}',30) AND '${outFileSuffix}' and terminal in (15,16,20,103,182,80,79,78,141)
            ) a
            ...
            inner join (
                ...
                where aa.dt = '${outFileSuffix}' and to_date(aa.pay_time) between date_sub('${outFileSuffix}',30) AND '${outFileSuffix}'

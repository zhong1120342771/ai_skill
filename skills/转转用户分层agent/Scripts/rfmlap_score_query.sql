-- 转转用户分层 RFMLAP 评分 SQL 模板
-- 占位符：${dt} = 统计日期 YYYY-MM-DD
-- 引擎：星河 Hive engine=5
-- 详细注释见完整版 Scripts/rfmlap_score_query_full.sql
...
order_rfm AS (
    ...
    WHERE dt >= DATE_SUB('${dt}',180) AND dt <= '${dt}'
      AND is_pure_pay_success=1 AND order_structure_type IN (0,1) AND buyer_id IS NOT NULL
    GROUP BY buyer_id
),
order_p AS (
    SELECT buyer_id AS uid,
...
scored AS (
    SELECT u.token, u.uid, u.regist_days, u.user_type, u.user_source,
        ...
        -- P评分
        LEAST(3, CASE WHEN (CASE WHEN p.p_total_cnt>0 THEN p.p_coupon_cnt*1.0/p.p_total_cnt ELSE 0 END >=0.5)
...
SELECT '${dt}' AS dt, token, user_type, user_source, regist_days,
